...
with open('data\words.txt', 'r', encoding='utf-8') as file:
    ...
    importetais_vards_easy = [x for x in visi_vardi if len(x) <= 6]
    ...
    importetais_vards_medium = [x for x in visi_vardi if len(x)>=7 and len(x)<=25]
    ...
    importetais_vards_hard = [x for x in visi_vardi if len(x) >= 4]
